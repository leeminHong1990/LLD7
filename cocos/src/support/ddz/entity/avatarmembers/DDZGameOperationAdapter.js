...
var DDZGameOperationAdapter = DDZPlaybackOperationAdapter.extend({
	...
	postOperation: function (serverSitNum, aid, data, nextServerSitNum) {
		...
		if (aid === const_ddz.OP_PASS) {
			...
			cc.log("discard time:",Math.round(new Date() / 1000));
			cc.log("serverSitNum:",serverSitNum);
	...
	playDiscardSound: function (serverSitNum, data, lastDiscardIdx) {
		cc.log("playDiscardSound time:",Math.round(new Date() / 1000));
		var info = ddz_rules.test_with_rule(data, false, false, true, this.curGameRoom.flower_mode === const_ddz.MODE_HAS_FLOWER);
		if (!info[0]) {
			return
		}
		var lastType = null;
		if (lastDiscardIdx !== -1 && serverSitNum !== lastDiscardIdx) {
			...
			var lastInfo = ddz_rules.test_with_rule(last, false, false, true, this.curGameRoom.flower_mode === const_ddz.MODE_HAS_FLOWER);
			if (lastInfo[0]) {
				lastType = lastInfo[1];
			}
		}
