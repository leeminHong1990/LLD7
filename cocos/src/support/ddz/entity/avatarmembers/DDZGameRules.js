...
var DDZGameRules = DDZRoomOperationAdapter.extend({
	...
	compareCards: function (src, dest, converted, ignoreGreater) {
		...
		let srcInfo = ddz_rules.test_with_rule(src, true, true, true, hasFlower);
		if (!srcInfo[0]) {
			return -1;
		}
	...
	canDiscard: function (cards, serverSitNum) {
		...
		if (lastCards) {
			...
			// 可以出任意牌
			return ddz_rules.test_with_rule(cards, false, false, false, hasFlower);
		}
	},
	...
	getBestTip: function (selectCards) {
		...
		let hasFlower = this.curGameRoom.flower_mode === const_ddz.MODE_HAS_FLOWER;
		if (lastCards) {
			let lastInfo = ddz_rules.test_with_rule(lastCards, false, false, true, hasFlower);
			if (lastInfo) {
				lastType = lastInfo[1];
			}
		}
