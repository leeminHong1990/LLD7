...
var LL7GameRules = LL7RoomOperationAdapter.extend({
	...
	canDiscardPoker: function (pokers) {
        ...
        if((fd_suitPattern&3) === const_ll7.TYPE_ONE){
            ...
            let hand_fit_pairs = cutil_ll7.get_type_pokers(hand_fit_pokers, const_ll7.TYPE_PAIR);
            return hand_fit_pairs.length>0 ? collections.reduce(hand_fit_pairs, 1).sort(collections.compare) : hand_fit_pokers.reverse()
	...
	tips:function (pokers) {
        ...
        if (!fd_pokers || fd_pokers.length === 0) {
            ...
            return collections.reduce(_pokers, 2);
        }
        ...
        let can_discard_pokers = this.canDiscardPoker(pokers).concat([]).reverse();
        ...
        if((fd_suitPattern&3) === const_ll7.TYPE_ONE){
            ...
            merge_pair_one.sort(cutil_ll7.large_sort);
