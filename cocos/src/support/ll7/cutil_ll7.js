...
cutil_ll7.get_type_pokers = function (pokers, type) {
	...
	if(type === const_ll7.TYPE_ONE){
		// 特殊处理7
		keys.sort(cutil_ll7.large_sort);
        ...
        // 特殊处理7
        pair_keys.sort(cutil_ll7.large_sort);
...
// 按实际大小排序， 7 特殊处理
cutil_ll7.large_sort = function (a, b) {
    if(const_ll7.SEVEN.indexOf(a) >= 0){
        a = a+36
    }
    if(const_ll7.SEVEN.indexOf(b) >= 0){
        b = b+36
    }
    return a-b
};