...
var LL7PlayerInfoSnippet = UISnippet.extend({
	...
	defaultUiGetter: function (serverSitNum, name) {
		...
		let idx = h1global.player().server2CurSitNum(serverSitNum);
		if (idx === 2 || idx === 1) {
			cur_player_info_panel.getChildByName("grab_panel").setPositionX(-80);
		}
	...
	update_friend_idx: function (isFriend, runAction) {
		...
		var friend_img = this.uiGetter(this.serverSitNum, "friend_img");
		if (isFriend) {
			this.uiGetter(this.serverSitNum, "foreman_img").setVisible(false);
		}

		friend_img.setVisible(isFriend);
		if(runAction){
			friend_img.setVisible(false);
		}
		// if (runAction) {
		// 	let player = h1global.player();
		// 	if(!player || !player.curGameRoom){
		// 		return;
		// 	}
		// 	friend_img.stopAllActions();
		// 	friend_img.setVisible(false);
		// 	// load_idx
		// 	var dealer_img = this.uiGetter(player.curGameRoom.lordIdx, "friend_img");
		// 	dealer_img.setVisible(true);
		// 	var old_pos = dealer_img.getPosition();
		// 	var goto_pos = this.rootUINode.getPosition();
		// 	// dealer_img.runAction()
		// 	var win_size = cc.director.getWinSize();
		// 	dealer_img.runAction(cc.sequence(
		// 		// cc.moveTo(3,win_size.width*0.5,win_size.height*0.5),
		// 		cc.moveTo(3,goto_pos.x,goto_pos.y),
		...
		//
		//
		// 	// friend_img.setScale(5);
		// 	// friend_img.runAction(cc.sequence(cc.scaleTo(0.5, 1), cc.scaleTo(1, 1)));
		// }
	},
