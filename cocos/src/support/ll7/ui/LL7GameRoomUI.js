...
var LL7GameRoomUI = UIBase.extend({
	...
	update_wait_time_left: function (leftTime) {
		...
		if (player && player.curGameRoom) {
			...
			lefttime_label = this.rootUINode.getChildByName("operation_wait_panel").getChildByName("left_time_label");
	...
	_setBeginGameShow: function (is_show, myServerSitNum) {
		...
		if (!is_show) {
			...
			let player = h1global.player();
			if (player && player.curGameRoom) {
				this.update_player_hand_tiles(serverSitNum, player.curGameRoom.handTilesList[serverSitNum] ,undefined,1);
	...
	update_friend_idx: function (friendIdx, runAction) {
		...
		if(runAction){
			...
			// load_idx
			var dealer_img = this.containUISnippets["PlayerInfoSnippet" + player.curGameRoom.mainServerSitNum].uiGetter(player.curGameRoom.mainServerSitNum, "friend_img");

			var friend_img = this.containUISnippets["PlayerInfoSnippet" + friendIdx].uiGetter(friendIdx, "friend_img");

			var d_p = friend_img.convertToWorldSpaceAR();
			var f_p = dealer_img.convertToNodeSpaceAR(d_p);

			dealer_img.setVisible(true);
			var old_pos =dealer_img.getPosition();
			dealer_img.stopAllActions();
			dealer_img.runAction(cc.sequence(
				cc.moveBy(1,f_p.x,f_p.y),
				cc.callFunc(function(){
					friend_img.setVisible(true);
					dealer_img.setVisible(false);
					dealer_img.setPosition(old_pos);
				})
			));
		}
	},
	...
	update_player_desk_tiles: function (serverSitNum, tileList, myServerSitNum, curGameRoom) {
		...
		for (var i = 0; i < const_ll7.DESK_CARD_NUM; i++) {
			...
			if (num > 0) {
				...
				tile.getChildByName("mark_star").setVisible(false);
		...
		if (imgName) {
			...
			if (idx == 1 || idx == 2) {
				if(tileList.length<=2){
					...
					node.setPositionX(850 - parseFloat(tileList.length) * 40 - 25 -85);
				}else{
				...
				if(tileList.length<=2){
					node.setPositionY(30);
					node.setPositionX(parseFloat(tileList.length) * 40 + 25 + 85);
				}else{
	 ...
	 * @param end
	 */
	update_player_hand_tiles: function (serverSitNum, tileList, limit ,end) {
		if (!this.is_show) {
			return;
		}
		let player = h1global.player();
		...
		// if(this.beginAnimPlaying || end){
			var flag_list = [];
			for (var i = 0; i < const_ll7.HAND_CARD_NUM; i++) {
				let tile = cur_player_tile_panel.getChildByName('tile_img_' + i);
				if(tile.selectedFlag){
					if(i>=1){
						var g_tile = cur_player_tile_panel.getChildByName('tile_img_' + (i-1));
						if(!g_tile.selectedFlag && g_tile.card === tile.card){
							flag_list.push(tile.card * 10);
							continue;
						}
					}
					flag_list.push(tile.card);
				}
			}
		// }
		for (var i = 0; i < const_ll7.HAND_CARD_NUM; i++) {
		...
		// if(this.beginAnimPlaying || end){
			var list = cur_player_tile_panel.getChildren();
			this.selectedList = [];
			for(var k in list){
				if(list[k].card){
					var find =flag_list.indexOf(list[k].card);
					if(find>-1){
						this._toggleCard(list[k]);
						this.selectedList.push(list[k].card);
						flag_list.splice(find,1);
					}
					var find_2 = flag_list.indexOf(list[k].card * 10);
					if(find_2>-1){
						flag_list.splice(find_2,1,list[k].card);
					}
				}
			}
		// }
	...
	init_new_operation_panel: function () {
		var operation_panel = this.rootUINode.getChildByName("new_operation_panel");
		operation_panel.getChildByName("shou7_btn").addClickEventListener(UICommonWidget.touchEventVisibleCheckListener(this._doNewOperation(0)));
		operation_panel.getChildByName("jiaozhu_btn").addClickEventListener(UICommonWidget.touchEventVisibleCheckListener(this._doNewOperation(1)));
		operation_panel.getChildByName("fanzhu_btn").addClickEventListener(UICommonWidget.touchEventVisibleCheckListener(this._doNewOperation(2)));
	},
	...
	_doNewOperation: function (op_id) {
		var self = this;
		return function () {
			var player = h1global.player();
			if (!(player && player.curGameRoom)) {
			...
			var type = -1;// 这里要想办法获取type
			if(!self.beginAnimPlaying){
				let cur_player_tile_panel = self.rootUINode.getChildByName("player_hand_panel" + 0);
				self.selectedList = [];
				for (var i = 0; i < const_ll7.HAND_CARD_NUM; i++) {
					let tile = cur_player_tile_panel.getChildByName('tile_img_' + i);
					if(tile.selectedFlag){
						self.selectedList.push(tile.card);
					}
				}
			}
			if(op_id == 0 && !self.beginAnimPlaying){
				let cur_player_tile_panel = self.rootUINode.getChildByName("player_hand_panel" + 0);
				self.selectedList = [];
				let tile = cur_player_tile_panel.getChildren()[0];
				self.selectedList.push(tile.card);
			}
			var num = self.selectedList.length;
			if(num > 2  || num === 0){
				return;
			}
			if(num === 1){
				let poker = self.selectedList[0];
			...
			if(num === 2){
				let poker = self.selectedList[0];
				let poker2 = self.selectedList[1];
				if(poker!==poker2){
					return;
				}
				if (const_ll7.JOKERS.indexOf(poker) !== -1) {
	...
	init_game_info_panel: function () {
		...
		// this.rootUINode.getChildByName("bottom_bar_panel").getChildByName("getscore").getChildByName("score").setString(player.curGameRoom.mul_level);
	...
	update_main_panel: function (pokers) {
		...
		var root = this.rootUINode.getChildByName("main").getChildByName("cards");
	...
	update_operation_panel: function (tileList, state, mainPokers) {
		...
		var shou_7 = tileList.length === 1 && have_7;
		if(shou_7){
			this.selectedList = [tileList[0]];
		}
		var panel = this.rootUINode.getChildByName("new_operation_panel");

		if(have_7){
			panel.setVisible(true);
			if (state === const_ll7.LORD_FIRST || state === const_ll7.LORD_SECOND) {
				if (shou_7) {
					panel.getChildByName("shou7_btn").setVisible(true);
					panel.getChildByName("jiaozhu_btn").setVisible(false);
					panel.getChildByName("fanzhu_btn").setVisible(false);
					return;
				}
				panel.getChildByName("shou7_btn").setVisible(false);
				panel.getChildByName("jiaozhu_btn").setVisible(mainPokers.length === 0);
				panel.getChildByName("fanzhu_btn").setVisible(mainPokers.length !== 0);
				if(count7s.indexOf(2)===-1){
					panel.getChildByName("fanzhu_btn").setVisible(false);
				}
			}
			else if(state == const_ll7.DRAW_COVER || state == const_ll7.COVER_POKER){
					panel.getChildByName("shou7_btn").setVisible(false);
					panel.getChildByName("jiaozhu_btn").setVisible(false);
					panel.getChildByName("fanzhu_btn").setVisible(false);
			} else if (state === const_ll7.LORD_THIRD) {
				panel.getChildByName("jiaozhu_btn").setVisible(false);
				panel.getChildByName("fanzhu_btn").setVisible(true);
			}
		}else{
			panel.setVisible(false);
			panel.getChildByName("shou7_btn").setVisible(false);
	...
	show_operation_panel: function () {
		...
		let operation_panel = this.rootUINode.getChildByName("new_operation_panel");
	...
	hide_operation_panel: function () {
		...
		this.rootUINode.getChildByName("new_operation_panel").setVisible(false);
	},
	...
	startGame: function () {
		...
		if (player && player.curGameRoom) {
			...
			if (curGameRoom.lordAid === const_ll7.LORD_THIRD) {
				...
				if( player.serverSitNum === curGameRoom.lordIdx){
				...
				this.set_clock_pos(curGameRoom.curPlayerSitNum);
				this.update_all_player_desk_tiles();
			}
			for (var i = 0; i < const_ll7.MAX_PLAYER_NUM; i++) {
				let handTilesList = curGameRoom.handTilesList[i];
	...
	play_update_score_anim: function (serverSitNum, score) {
		...
		this.update_total_score_panel(total);
		// var self = this;
	...
	update_total_score_panel: function (score) {
		this.rootUINode.getChildByName("bottom_bar_panel").getChildByName("getscore").getChildByName("score").setString(score);
	},
	...
	update_operation_wait_panel: function (serverSitNum, state, pokers, nextServerSitNum) {
		...
		function updateFunc(imgName) {
			...
			wait_panel.getChildByName("wait_img").ignoreContentAdaptWithSize(true);
			wait_panel.stopAllActions();
			wait_panel.setVisible(true);
			wait_panel.runAction(createAnim());
			//在这里添加finger动画
			var hint_panel = wait_panel.getChildByName("hint_panel");
			hint_panel.setVisible(false);
			// if(imgName == "jiaozhu_img" || imgName == "fanzhu_img"){
			// 	hint_panel.setVisible(true);
			// 	hint_panel.getChildByName("finger_img").stopAllActions();
			// 	hint_panel.getChildByName("finger_img").setPositionY(0);
			// 	hint_panel.getChildByName("finger_img").runAction(cc.RepeatForever.create(cc.Sequence.create(
			// 		cc.moveBy(0.5,0,8),
			// 		cc.moveBy(0.5,0,-8)
			// 	)));
			// 	if(imgName == "fanzhu_img"){
			// 		hint_panel.getChildByName("hint_img").loadTexture("LL7GameRoomUI/hint_img2.png", ccui.Widget.PLIST_TEXTURE);
			// 	}else{
			// 		hint_panel.getChildByName("hint_img").loadTexture("LL7GameRoomUI/hint_img.png", ccui.Widget.PLIST_TEXTURE);
			// 	}
			// }else{
			// 	hint_panel.getChildByName("finger_img").stopAllActions();
			// 	hint_panel.setVisible(false);
