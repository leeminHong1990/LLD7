...
var LL7PlaybackGameRoom = AbstractPlaybackGameRoom.extend({
	...
	//埋底ui显示
	init_maidi_poker_tiles: function () {
		let player = h1global.player();
		if (!player || !player.curGameRoom) {
			return;
		}
		if(player.curGameRoom.maidiPokers.length <=0){
			return;
		}
		var is_double = player.curGameRoom.maidiPokers.length > 1;
		var tiles = is_double ? player.curGameRoom.maidiPokers[0][1].concat(player.curGameRoom.maidiPokers[1][1])  : player.curGameRoom.maidiPokers[0][1];
		let rootPanel = ccui.ImageView.create("res/ui/LL7GameRoomUI/playback_maidi_bg.png");
		rootPanel.setName("maidi_poker_tiles");
		this.roomUI.rootUINode.addChild(rootPanel);
		for (var i = 0; i < tiles.length; i++) {
			let card = tiles[i];
			//image size  23 * 48
			let ui = ccui.ImageView.create(this._getPokerImgPath(card), ccui.Widget.PLIST_TEXTURE);
			let line = parseInt(i / 8);
			ui.setPosition((i - line * 8) * 22 + 22 / 2.0 + 41, 82 - line * 50);
			ui.setName("tile_img_" + i);
			rootPanel.addChild(ui);
		}
		//屏幕位置
		rootPanel.setAnchorPoint(1, 0);
		rootPanel.setPosition(cc.winSize.width - 150, cc.winSize.height * 0.84);
	},

	_getPokerImgPath: function (card) {
		let rank = cutil_ll7.get_rank(card);
		let suit = cutil_ll7.get_suit(card);
		// 大小王
		let index = const_ll7.JOKERS.indexOf(card);
		if (index !== -1) {
			return "Poker/pic_poker_small_" + const_ll7.JOKERS_IMAGE_INDEX[index] + '.png';
		}
		if (rank > 13) {
			rank -= 13;
		}
		return "Poker/pic_poker_small_" + const_ll7.POKER_COLOR_DICT[suit] + "" + rank + '.png';
	},
});
