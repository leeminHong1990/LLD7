...
var LLKDDMJGameRoomEntity = GameRoomEntity.extend({
	ctor : function(player_num, gameType)
	{
		...
		this.discardStateList = [const_llkddmj.DISCARD_FREE, const_llkddmj.DISCARD_FREE, const_llkddmj.DISCARD_FREE, const_llkddmj.DISCARD_FREE]
		...
		this.pass_win_list = [const_llkddmj.NO_PASS_WIN,const_llkddmj.NO_PASS_WIN,const_llkddmj.NO_PASS_WIN,const_llkddmj.NO_PASS_WIN]; //玩家是否过胡
  		...
  		for(var i = 0; i < recRoomInfo["player_advance_info_list"].length; i++){
  			...
  			for(var j = 0; j < curPlayerInfo["op_list"].length; j++){
  				...
  				if(op_info["opId"] === const_val.OP_PONG){
                    ...
                    // cutil_llkddmj.tileSort(sortTiles, this.kingTiles);
