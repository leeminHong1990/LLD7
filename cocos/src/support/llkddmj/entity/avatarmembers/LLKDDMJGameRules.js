...
/*-----------------------------------------------------------------------------------------
 interface
 -----------------------------------------------------------------------------------------*/
var LLKDDMJGameRules = LLKDDMJRoomOperationAdapter.extend({

    getCanTingTiles:function () {
        var handTiles = this.curGameRoom.handTilesList[this.serverSitNum].concat([]);
	    var kingClassified = cutil_llkddmj.classifyKingTiles(handTiles, this.curGameRoom.kingTiles);
        ...
        for (var n = 0 ; n < handTiles.length ; n++) {
            ...
            for (var m = 0 ; m < canWinTiles.length ; m++) {
                ...
                if (this.curGameRoom.kingTiles.indexOf(t) >= 0) {
                    if (this.curGameRoom.game_mode === const_llkddmj.KING_GAME_MODE && this.curGameRoom.ting_mouse === 1) {
	                    var tempHandTiles = handTilesCopy.concat([t]);
	                    if (kingTilesNum === 0) {
		                    if (cutil_llkddmj.canNormalWinWithKing3N2(tempHandTiles, kingTilesNum)) {
			                    cc.log("can ting_mouse!");
		                    } else if (cutil_llkddmj.checkIs7Pairs(tempHandTiles, kingTilesNum) && (this.curGameRoom.game_mode === const_llkddmj.KING_GAME_MODE && this.curGameRoom.seven_pair === 1)) {
			                    cc.log("can ting_mouse!");
		                    } else {
			                    continue;
		                    }
	                    } else {
		                    if (cutil_llkddmj.canNormalWinWithKing3N2(tempHandTiles, kingTilesNum - 1)) {
			                    cc.log("can ting_mouse!");
		                    } else if (cutil_llkddmj.checkIs7Pairs(tempHandTiles, kingTilesNum - 1) && (this.curGameRoom.game_mode === const_llkddmj.KING_GAME_MODE && this.curGameRoom.seven_pair === 1)) {
			                    cc.log("can ting_mouse!");
		                    } else {
			                    continue;
		                    }
    ...
    getDrawOpDict: function (drawTile, serverSitNum) {
        ...
        if (this.isOpLimit()) {
            //胡
            if (handTiles.length % 3 === 2 && this.canWin(handTiles, drawTile, serverSitNum, const_val.OP_DRAW_WIN) && this.curGameRoom.discardStateList[serverSitNum] === const_llkddmj.DISCARD_FORCE) {
						    ...
						    for (var m = 0 ; m < canWinTilesAft.length ; m++) {
							    if (this.curGameRoom.kingTiles.indexOf(canWinTilesAft[m]) >= 0) {
								    // if (this.curGameRoom.game_mode === const_llkddmj.KING_GAME_MODE && this.curGameRoom.ting_mouse === 1) {
									 ...
									 //    var kingClassified = cutil_llkddmj.classifyKingTiles(tempHandTiles, this.curGameRoom.kingTiles);
									 //    var kings   = kingClassified[0];
									 //    var handTilesButKing  = kingClassified[1];
									 //    handTilesButKing.push(canWinTilesAft[m]);
									 //    var kingTilesNum = kings.length;
									 //    if (cutil_llkddmj.canNormalWinWithKing3N2(handTilesButKing, kingTilesNum - 1) || cutil_llkddmj.canNormalWinWithKing3N2(handTilesButKing, 0)) {
        ...
        //暗杠
        var tile2NumDict = cutil_llkddmj.getTileNumDict(handTiles)
    ...
    canWin:function(handTiles, finalTile, serverSitNum, aid){
        ...
        if(this.nomalCanWin(handTiles, finalTile, serverSitNum, aid)){
            ...
            //过胡只能自摸
            if (aid !== const_val.OP_DRAW_WIN && this.curGameRoom.pass_win_list[this.serverSitNum] === const_llkddmj.PASS_WIN) {
                return false;
            }

            //如果最后一张是财神也ok
            if(this.curGameRoom.kingTiles.indexOf(finalTile) >= 0 && aid === const_val.OP_DRAW_WIN){
	        ...
	        if (aid === const_val.OP_DRAW_WIN && this.curGameRoom.game_mode === const_llkddmj.KING_GAME_MODE && this.curGameRoom.mouse_general === 1 && this.curGameRoom.mouse_general_onetwo === 1) {
		        if (cutil_llkddmj.isMouseGeneral(handTiles,
				        cutil_llkddmj.classifyKingTiles(handTiles, this.curGameRoom.kingTiles)[1],
				        cutil_llkddmj.classifyKingTiles(handTiles, this.curGameRoom.kingTiles)[0].length,
    ...
    nomalCanWin: function (handTiles, finalTile, serverSitNum, aid) {
        ...
        var kingClassified = cutil_llkddmj.classifyKingTiles(handCopyTile, this.curGameRoom.kingTiles);
        var kings   = kingClassified[0];
        var handTilesButKing  = kingClassified[1];

        var kingTilesNum = kings.length;

	    if (((this.curGameRoom.game_mode === const_llkddmj.KING_GAME_MODE && this.curGameRoom.seven_pair === 1) || this.curGameRoom.game_mode !== const_llkddmj.KING_GAME_MODE) && cutil_llkddmj.checkIs7Pairs(handTilesButKing, kingTilesNum)) {              // 7对
	        return true;
        } else if (this.curGameRoom.game_mode === const_llkddmj.SPECIAL_GAME_MODE && cutil_llkddmj.getThirteenOrphans(handTilesButKing, kingTilesNum)) { // 十三幺
            return true;
        } else if(kingTilesNum > 0){                                            // 有癞子
            return cutil_llkddmj.canNormalWinWithKing3N2(handTilesButKing, kingTilesNum);
        } else {                                                                // 没癞子
            return cutil_llkddmj.canNormalWinWithoutKing3N2(handTilesButKing);
        }
        // }
    },
