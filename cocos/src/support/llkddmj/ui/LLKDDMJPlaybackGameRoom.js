...
//
"use strict"
var LLKDDMJPlaybackGameRoom = cc.Node.extend({
    ...
    init: function () {
        ...
        this.roomUI.show_ting_panel = function(is_show) {
    ...
    doNext: function () {
        ...
        if (commands.length > 0) {
            ...
            if (player) {
	            ...
	            if (command.aid === const_llkddmj.DISCARD_FORCE) {
		            let player = h1global.player();
		            player.curGameRoom.discardStateList[command.serverSitNum] = const_llkddmj.DISCARD_FORCE;
		            this.roomUI.update_player_exposed_tiles(command.serverSitNum, player.curGameRoom.handTilesList[command.serverSitNum], true);
		            this.roomUI.playOperationEffect(const_val.OP_READY, command.serverSitNum, command.tileList);
		            if (player.curGameRoom.playerInfoList[command.serverSitNum]["sex"] === 1) {
			            cutil.playEffect(player.gameOperationAdapter.gameType, 'male/sound_man_ting.mp3');
		            } else {
			            cutil.playEffect(player.gameOperationAdapter.gameType, 'female/sound_woman_ting.mp3');
		            }
		            return;
	            }
	            ...
	            if (commands.length > 0) {
		            ...
		            player.gameOperationAdapter.waitForOperationFromNext(last.serverSitNum, last.aid, last.tileList)
	            }
            }
        ...
        if (op_special_record) {
            ...
            while (flag) {
                ...
                if (re[3] == this.totalCommand - this.commands.length) {
                    ...
                    player.postPlayerDiscardState(re[1], const_llkddmj.DISCARD_FORCE);
