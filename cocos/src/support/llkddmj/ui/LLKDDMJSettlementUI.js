"use strict"
var LLKDDMJSettlementUI = SettlementUI.extend({
    show_by_info: function (roundRoomInfo, serverSitNum, curGameRoom, confirm_btn_func, replay_btn_func) {
        ...
        this.show(function(){
            ...
            for(var i = 0; i < 4; i++){
                ...
                self.update_player_win(i, server_seat_num, roundRoomInfo["win_idx"], roundRoomInfo["from_idx"], roundRoomInfo["dealer_idx"], roundRoomInfo["result_list"], curGameRoom.game_mode, roundRoomInfo["finalTile"], curGameRoom, roundPlayerInfo["concealed_kong"], roundRoomInfo["lastWinScore"]);
            }
	...
	show_rules : function (curGameRoom) {
		var roominfo_list = [["普通玩法","特殊牌型玩法","捉耗子"],["风耗子", "单耗子", "双耗子"],["","赏金"],["","七小对"],["","耗子吊将"],["","耗子吊将1、2能胡"],["","可以单听耗子"],["","特殊牌型翻倍"]];
		var share_list = [];
		...
		if (curGameRoom.base_score >= 1) {
			share_list.push("倍数" + (curGameRoom.base_score).toString() + "倍");
		}
    ...
    update_player_hand_tiles:function(panel_idx, serverSitNum, curGameRoom ,tileList, win_idx, finalTile, concealedKongList){
        ...
        if(win_idx == serverSitNum) {
            tileList.pop();
            cutil_llkddmj.tileSort(tileList, curGameRoom.kingTiles);
        ...
        for(var i = 0; i < 14; i++){
            ...
            if(tileList[i]){
                ...
                if(win_idx == serverSitNum && i == tileList.length - 1){
                    ...
                    var hu_img = ccui.ImageView.create("res/ui/SettlementUI/hu_title_img.png");
                    hu_img.ignoreContentAdaptWithSize(true);
                    hu_img.setScale(2.0);
                    tile_img.addChild(hu_img);
                    hu_img.setPosition(cc.p(100, 109));
                }
    ...
    update_player_win:function(panel_idx, serverSitNum, win_idx, from_idx, dealer_idx, result, game_mode, finalTile, curGameRoom, concealedKongList, lastWinScore){
        var cur_player_info_panel = this.player_tiles_panels[panel_idx];
        ...
        if(serverSitNum === win_idx) {
            if (game_mode === const_llkddmj.SPECIAL_GAME_MODE) {
	    ...
	    if (curGameRoom.add_dealer === 1) {
		    if (dealer_idx == serverSitNum && win_idx == serverSitNum) {
			    //庄家赢
			    item_win_type_label.string += "\t\t\t加庄+10";
	    ...
	    for (var i = 0; i < exposedKongList.length; i++) {
		    ...
		    if (curGameRoom.kingTiles.indexOf(exposedKongList[i]) >= 0 && game_mode == const_llkddmj.KING_GAME_MODE) {
			    kongScore = 50;
		    }
	    ...
	    for (var i = 0; i < concealedKongList.length; i++) {
		    ...
		    if (curGameRoom.kingTiles.indexOf(concealedKongList[i]) >= 0 && game_mode == const_llkddmj.KING_GAME_MODE) {
			    kongScore = 50;
		    }
	    ...
	    //倍数
	    if (curGameRoom.base_score > 1 && serverSitNum === win_idx) {
		    item_win_type_label.string += "\t\t\t倍数x" + (curGameRoom.base_score).toString();
	    }
