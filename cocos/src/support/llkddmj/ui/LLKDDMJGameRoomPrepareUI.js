"use strict";
var LLKDDMJGameRoomPrepareUI = UIBase.extend({
    ctor:function() {
        this._super();
        this.resourceFilename = "res/ui/LLKDDMJGameRoomPrepareUI.json";
        this.talk_img_num = 0;
    },
    ...
    initUI:function(){
        ...
        // var share_title = cutil.get_share_title(player.curGameRoom,const_val.LvLiangKDDMJ);
        var share_title = '';
        ...
        if(player.curGameRoom["game_mode"]!=2){
	        ...
	        player.curGameRoom["seven_pair"] = 99;
	        player.curGameRoom["mouse_general"] = 99;
	        player.curGameRoom["mouse_general_onetwo"] = 99;
	        player.curGameRoom["ting_mouse"] = 99;
        }
	    if (player.curGameRoom["game_mode"]!=1){
		    player.curGameRoom["special_mul"] = 99;
	    }

        var share_desc = cutil.get_share_desc(player.curGameRoom,const_val.LvLiangKDDMJ);
        ...
        wxinvite_btn.addTouchEventListener(function(sender, eventType){
            if(eventType == ccui.Widget.TOUCH_ENDED){
                ...
                share_title = cutil.get_share_title(player.curGameRoom,const_val.LvLiangKDDMJ);
    ...
    get_share_desc : function () {
        ...
        var roominfo_list = [["普通玩法","特殊牌型玩法","捉耗子"],["风耗子", "单耗子"],["",""]];
        var share_list = [];
	    ...
	    if (player.curGameRoom.base_score >= 1) {
		    share_list.push("倍数" + player.curGameRoom.base_score.toString() + "倍");
	    }
    ...
    // init_quite_panel:function(){
    //    var quite_panel = this.rootUINode.getChildByName("gameprepare_panel").getChildByName("quite_panel");
    //
    ...
    //    quite_panel.getChildByName("close_btn").addTouchEventListener(quite_btn_event);
    //
    //    var player = h1global.player();
