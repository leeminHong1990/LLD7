...
"use strict"
var LLKDDMJGameRoomUI = UIBase.extend({
	ctor:function() {
		this._super();
		this.talk_img_num = 0;
	},
    initUI:function(){
        ...
        this.touch_state = const_llkddmj.PLAYER_TOUCH_SELF_STATE
    ...
    init_ting_panel:function(){
        ...
        function ting_btn_event(sender, eventType){
            if (eventType === ccui.Widget.TOUCH_ENDED) {
                ...
                // player.setDiscardState(const_llkddmj.DISCARD_FORCE);
                // self.checkTing = true;
	            h1global.curUIMgr.roomLayoutMgr.iterUI(function (ui) {
		            ui.checkTing = true;
	            });
        ...
        function guo_btn_event(sender, eventType){
            if (eventType === ccui.Widget.TOUCH_ENDED) {
                var player = h1global.entityManager.player();
                player.setDiscardState(const_llkddmj.DISCARD_FREE);
	            h1global.curUIMgr.roomLayoutMgr.iterUI(function (ui) {
		            ui.checkTing = false;
	            });
                ...
                h1global.curUIMgr.roomLayoutMgr.notifyObserver("hide_operation_panel");
                // ting_btn.setVisible(true);
	            h1global.curUIMgr.roomLayoutMgr.notifyObserver("show_ting_btn", true);
                // self._setTileNoMask();
	            h1global.curUIMgr.roomLayoutMgr.notifyObserver("_resetSelectTile");
    ...
    _setTileMask:function () {
        ...
        for( var i = 0 ; i < handTiles.length ; i++) {
            ...
            if (player.gameOperationAdapter.canTingTiles.indexOf(handTiles[i]) < 0) {
                tile_img.color = const_llkddmj.mark_same_color;
            }
        }
    },
    ...
    _setTileNoMask:function () {
        ...
        for( var i = 0 ; i < handTiles.length ; i++) {
            var tile_img = cur_player_hand_panel.getChildByName("tile_img_" + i.toString());
            tile_img.color = const_llkddmj.mark_none_color;
        }
    },
    ...
    force_discard:function () {
        ...
        // return;
        // var player = h1global.player();
        // this.touch_state = const_llkddmj.PLAYER_TOUCH_FORCE_STATE;
        ...
        if(this.isNeedDiscard &&  const_val.FAKE_COUNTDOWN > 0 && player.curGameRoom.handTilesList[player.serverSitNum].length%3===2){
            if(!this.rootUINode.getChildByName("need_discard_panel").isVisible()){
                ...
                this.rootUINode.runAction(cc.repeat(cc.sequence(cc.callFunc(function () {
                    ...
                    if (p && p.curGameRoom && p.curGameRoom.curPlayerSitNum === p.serverSitNum && self.rootUINode.getChildByName("need_discard_panel").isVisible()) {
    ...
    _resetSelectTile: function () {
        ...
        if (this.curSelectTile) {
            ...
            this.update_canwin_tile_panel(const_llkddmj.NOT_DISPLAY_CANWIN_PANEL);
        }
    },
    ...
    init_player_tile_panel:function(){
        ...
        function unchoose_tile() {
            ...
            self.update_canwin_tile_panel(const_llkddmj.NOT_DISPLAY_CANWIN_PANEL);
        }
        ...
        function discard_tile(tile, discard_pos) {
            ...
            if (player){
                ...
                if (self.checkTing && player.gameOperationAdapter.canTingTiles.indexOf(tile.tileNum) >= 0) {
                    player.setDiscardState(const_llkddmj.DISCARD_FORCE, tile.tileNum);
        ...
        function hand_tile_event(tile, touch_mode, sender){
            ...
            if (self.touch_state === const_llkddmj.PLAYER_TOUCH_SELF_STATE && !player.gameOperationAdapter.canDiscardIdx(tile.index)){ //别人打财神后 限制出牌
                return
            }
	        if (self.checkTing && player.gameOperationAdapter.canTingTiles.indexOf(tile.tileNum) < 0) {
		        return;
            ...
            if(touch_mode == ccui.Widget.TOUCH_BEGAN){ // 只选中
                ...
                if(self.choose_tile_list.length == 2 && self.choose_tile_list[0].index == self.choose_tile_list[1].index && self.curSelectTile.index == self.choose_tile_list[0].index){
                    ...
                    if(self.touch_state !== const_llkddmj.PLAYER_TOUCH_SELF_STATE){ // 非出牌状态不能出牌
                        return
                    }
                    discard_tile(tile)
        ...
        function touch_func(touch_pos, eventType, sender){
            // cc.log(handTiles)
            let effective_width = self.touch_state === const_llkddmj.PLAYER_TOUCH_SELF_STATE ? config.real_width * handTiles.length + offset_x : config.real_width * handTiles.length
            if (touch_pos.x >= 0 && touch_pos.x < effective_width && touch_pos.y >= 0 && touch_pos.y <= hand_panel_height) {
                ...
                if (self.touch_state === const_llkddmj.PLAYER_TOUCH_SELF_STATE && touch_idx + 1 >= handTiles.length) { //3x+2 最后一张特殊处理
                ...
                if (self.touch_state !== const_llkddmj.PLAYER_TOUCH_SELF_STATE && self.curSelectTile) {
                    reset_tile()
                }
            }
        }
        ...
        function player_hand_panel_event(sender, eventType){
            // NOTE:下乡禁止触摸和滑动
            if(self.touch_state === const_llkddmj.PLAYER_TOUCH_FORCE_STATE || player.curGameRoom.discardStateList[player.serverSitNum] === const_llkddmj.DISCARD_FORCE){
                if(self.curSelectTile){
                    reset_tile()
                }
                return;
            ...
            if (eventType == ccui.Widget.TOUCH_BEGAN) {
	            if (player.gameOperationAdapter.canTingTiles) {
		            if (player.gameOperationAdapter.canTingTiles.length > 0) {
			            if (self.curSelectTile && player.gameOperationAdapter.canTingTiles.indexOf(self.curSelectTile.tileNum) < 0 && player.curGameRoom.discardStateList[player.serverSitNum] === const_llkddmj.DISCARD_FORCE) {
				            reset_tile()
				            return;
			            }
		            }
                ...
                touch_func(p, eventType, sender)
                if (self.curSelectTile && !self.moving_tile && self.touch_state === const_llkddmj.PLAYER_TOUCH_SELF_STATE && p.y - self.curSelectTile.getPositionY() > 76) { // 向上拖 生成一个 麻将子
                ...
                if (self.moving_tile) {
                    ...
                    // 出牌
                    if (py > hand_panel_height + config.sel_posy && self.touch_state === const_llkddmj.PLAYER_TOUCH_SELF_STATE) {
    ...
    mark_king_tile:function (tile_img) {
        tile_img.color = const_llkddmj.mark_king_color;
    },
    ...
    mark_discard_king_tiles:function () {
        ...
        for(var i = 0; i < list.length; i++){
            ...
            let start = 0 ;
            if(tileSum > const_llkddmj.MAX_DISCARD_TILES_SIZE){
                start = tileSum - const_llkddmj.MAX_DISCARD_TILES_SIZE;
            }
    ...
    mark_same_tiles:function(tileNum){
        ...
        for(var i = 0; i < list.length; i++){
            ...
            var maxDeskSize = const_llkddmj.MAX_DISCARD_TILES_SIZE;
            if (player.curGameRoom.discardStateList[i] === const_llkddmj.DISCARD_FORCE) {
    ...
    cancel_mark_same_tiles:function(){
        ...
        for(var i = 0; i < list.length; i++){
            ...
            for(var j = start; j< tileSum; j++){
                if(player.curGameRoom.kingTiles.indexOf(tiles[j]) < 0)	{
                    var tile_img = cur_player_discard_panel.getChildByName("tile_img_" + (j  - start));
                    if (tile_img.color.b === const_llkddmj.mark_ting_color.b) {
                        continue;
                    }
                    this.cancel_tile_color(tile_img);
                }
    ...
    lock_player_hand_tiles:function(){
        if(!this.is_show) {return;}
        var player = h1global.player();
        this.touch_state = const_llkddmj.PLAYER_TOUCH_OTHER_STATE;
    ...
    _setBeginGameShow:function(is_show){
        ...
        if (player) {
	        if (player.curGameRoom.kingTiles.length > 0) {
		        if (player.curGameRoom.kingTiles.length <= 1) {
			        this.rootUINode.getChildByName("kingtile_panel").setVisible(is_show);
		        } else if (player.curGameRoom.kingTiles.length >= 2) {
			        this.rootUINode.getChildByName("kingtile_double_panel").setVisible(is_show);
		        }
	        }
	        for (var i = 0; i < player.curGameRoom.player_num; i++) {
    ...
    update_player_hand_tiles: function (serverSitNum, tileList) {
        ...
        for (var i = 0; i < 14; i++) {
            ...
            if(handTilesList[i] !== undefined){
                if(handTilesList[i] > 0){
                    ...
                    if(isPlayer){
                        if(discardState === const_llkddmj.DISCARD_FORCE){
                            this.mark_same_tile(tile_img);
                            this.lock_player_hand_tiles();
                        }else{
                            this.cancel_tile_color(tile_img);
        ...
        if(serverSitNum == player.serverSitNum && player.curGameRoom.discardStateList[serverSitNum] === const_llkddmj.DISCARD_FREE){
        ...
        for (var i = 0; i < curUpNum; i++) {
            for (var j = 0; j < 3; j++) {
                ...
                if (cur_upTilesList[i][j]) {
                    ...
                    mahjong_img.setVisible(true);
                    if (player.curGameRoom.kingTiles.indexOf(cur_upTilesList[i][j]) >= 0 && kingtilemarkOptions) {
            ...
            if (cur_upTilesList[i].length > 3) {
                ...
                if (cur_upTilesList[i][3]) {
                    ...
                    mahjong_img.setVisible(true);
                    if (player.curGameRoom.kingTiles.indexOf(cur_upTilesList[i][3]) >= 0 && kingtilemarkOptions) {
	                    var kingtilemark_img = this._createKingTileMark(kingtilemarkOptions, i);
	                    this.upTileMarksList[idx].push(kingtilemark_img);
    ...
    play_discard_anim:function(serverSitNum, onlyTilePointer, buckle){
        ...
        if(discardTilesSum > const_llkddmj.MAX_DISCARD_TILES_SIZE) discardIndex = const_llkddmj.MAX_DISCARD_TILES_SIZE - 1;
    ...
    update_player_discard_tiles:function(serverSitNum){
        ...
        var deskSize = const_llkddmj.DISCARD_TILES_SIZE;
        var maxDeskSize = const_llkddmj.MAX_DISCARD_TILES_SIZE;
        ...
        var discardTilesList = player.curGameRoom.discardTilesList[serverSitNum];
        var tingTilePos = undefined;
        if (player.curGameRoom.discardStateList[serverSitNum] === const_llkddmj.DISCARD_FORCE) {
        ...
        for(var i = 0; i < deskSize; i++){
            ...
            if(i < maxDeskSize && discardTilesList[i] > 0){
                ...
                if (tingTilePos >= 0 && tingTilePos === i) {
                    if (isPlayer) {
                        // this.mark_same_tile(tile_img);
                        tile_img.color = const_llkddmj.mark_ting_color;
                    } else {
                        if (options.mahjongdesk) {
    ...
    update_wintips_btn:function () {
        ...
        if(canWinTiles.length <= 0 || player.curGameRoom.curPlayerSitNum == player.serverSitNum){
            ...
            wintips_btn.addTouchEventListener(function (sender, eventType) {
                if (eventType === ccui.Widget.TOUCH_BEGAN) {
                    self.update_canwin_tile_panel(const_llkddmj.WINTIPS_BTN_DISPLAY);
    ...
    update_canwin_tile_panel:function(select_tile){
        ...
        var self = this;
        var canwin_tile_panel = self.rootUINode.getChildByName("canwin_tile_panel");
        if(select_tile == const_llkddmj.NOT_DISPLAY_CANWIN_PANEL){
            canwin_tile_panel.setVisible(false);
            return;
        }
        var player = h1global.player();
        ...
        if(player.curGameRoom.kingTiles.length > 0) {
            for (var k = 0 ; k < player.curGameRoom.kingTiles.length ; k++) {
	            tile_img.getChildByName("mahjong_img").loadTexture("Mahjong/mahjong_big_" + player.curGameRoom.kingTiles[k].toString() + ".png", ccui.Widget.PLIST_TEXTURE);
	            tile_img.color = const_llkddmj.mark_same_color;
	            var tileSum = self.find_all_same_tiles(player.curGameRoom.kingTiles[k]);
    ...
    play_result_anim: function (player_info_list) {
        ...
        for (var i = 0; i < curGameRoom.player_num; i++) {
            ...
            cutil_llkddmj.tileSort(handTiles, player.curGameRoom.kingTiles);
    ...
    update_kingtile_panel:function(){
        ...
        if (player.curGameRoom.kingTiles.length > 1) {
	        kingtile_panel = this.rootUINode.getChildByName("kingtile_double_panel");
        }
        for (var i = 0 ; i < player.curGameRoom.kingTiles.length ; i++) {
	        var tile_img = kingtile_panel.getChildByName("tile_img_" + i.toString());
	        var kingtilemark_img = ccui.ImageView.create("res/ui/GameRoomUI/kingtilemark.png");
	        kingtilemark_img.setAnchorPoint(0.0, 1.0);
	        kingtilemark_img.setPosition(cc.p(4, 67));
	        kingtilemark_img.setScale(0.7);
	        tile_img.addChild(kingtilemark_img);
	        this.mark_king_tile(tile_img);
	        var mahjong_img = tile_img.getChildByName("mahjong_img");
	        mahjong_img.ignoreContentAdaptWithSize(true);
	        mahjong_img.loadTexture("Mahjong/mahjong_big_" + player.curGameRoom.kingTiles[i].toString() + ".png", ccui.Widget.PLIST_TEXTURE);
	        mahjong_img.setVisible(true);
	        this.mark_king_tile(tile_img);
        }
    ...
    update_operation_panel:function(op_dict, from_type){
        ...
        var self = this;
        var player = h1global.player();
        // if (player.curGameRoom.discardStateList[player.serverSitNum] === const_llkddmj.DISCARD_FORCE && player.curGameRoom.game_mode !== const_llkddmj.KING_GAME_MODE) {
