...
var ClubUI = UIBase.extend({
    ...
    init_top_panel:function () {
		...
		var setting_btn = top_panel.getChildByName("setting_btn");
		...
		setting_btn.addTouchEventListener(function (sender, eventType) {
			if(eventType === ccui.Widget.TOUCH_ENDED){
				if(eventType === ccui.Widget.TOUCH_ENDED){
					h1global.globalUIMgr.info_ui.show_by_info("今日对局数：" + self.club.club_base_info["dailyRound"] + "局");
				}
			}
		});
	...
	update_setting_btn:function(){
		...
		if(player){
			if(this.club.is_owner(player.userId)){
				top_panel.getChildByName("setting_btn").setVisible(true);
			}else{
				top_panel.getChildByName("setting_btn").setVisible(false);
			}
		}
	},
    ...
    init_bottom_panel:function () {
        ...
        if (player.userId === this.club.owner.userId) {
            ...
            setting_btn.setPositionX(bottom_panel.getContentSize().width * 0.275);
	...
	update_sic_info:function (club_id, event_msg ){
        ...
        this.set_title_right_label(title_panel,event_msg["seat_info"],this.club.is_owner(h1global.player().userId),this.club.club_base_info.table_info_list[idx]["room_state"]);

        //设置头像的显示与消失
        //var stool_img = curItem.getChildByName("stool_img");// 服务端传下数据，检测这种游戏最多有多少玩家
        //stool_img.setVisible(false);
		var player_num = const_val.GameType2CNum[this.club.club_base_info.table_info_list[idx]["game_type"]];
    ...
    update_member_is_apply:function (club_id,event_msg){
        ...
        if(this.club.club_id != club_id){return;}
        if(!h1global.player()){return;}
        if(this.club.is_owner(h1global.player().userId)==false){
            event_msg = 0;
        }
        var bottom_panel = this.rootUINode.getChildByName("bottom_panel");
        var mem_btn = bottom_panel.getChildByName("mem_btn");
	...
	update_desk_details:function (view, info, index) {
        ...
        stool_img.setVisible(false);
        var player_num = const_val.GameType2CNum[info["game_type"]];
        ...
        title_bg_btn.addTouchEventListener(function (sender, eventType) {
            if(eventType === ccui.Widget.TOUCH_ENDED){
                ...
                if(self.club.is_owner(p.userId) || self.club.club_base_info["r_switch"]){
                    if(h1global.curUIMgr.clubmode_ui && !h1global.curUIMgr.clubmode_ui.is_show){
        ...
        self.set_title_right_label(title_panel,info["seat_info"],self.club.is_owner(h1global.player().userId),info["room_state"]);
        view.getChildByName("cur_round_label").setVisible(false);
    ...
    update_desk_title_label:function(club_id,event_msg){
        ...
        var is_owner = this.club.is_owner(h1global.player().userId);
    ...
    check_lock_panel:function(club_id){
        ...
        if(h1global.player() && h1global.player().userId === this.club.owner.userId){
