...
var InfoUI = UIBase.extend({
	...
	show_by_info_with_size_in_cs:function(content, c_size){
		//客服显示专用
		// if (hide_on_close === undefined) {
		// 	hide_on_close = true;
		// }
		...
		this.show(function(){
			...
			// self.info_panel.getChildByName("return_btn").setPosition(cc.p((c_size.width + 70) * 0.95, (c_size.height + 70) * 0.95));
			var content_label = self.info_panel.getChildByName("content_label");
			content_label.setPosition(cc.p((c_size.width + 70) * 0.5, (c_size.height + 130) * 0.5));
			content_label.setContentSize(c_size);
			content_label.setString(content);
			var return_btn = self.info_panel.getChildByName("return_btn");
			return_btn.setPositionX(self.info_panel.getContentSize().width * 0.5);
		});
	},
