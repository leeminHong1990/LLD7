...
var ConfigUI = BasicDialogUI.extend({
	...
	update_state: function () {
		...
		if (player && player.curGameRoom) {
			...
			if(player.curGameRoom.gameType == const_val.LvLiang7){
				this.gameconfig_panel.getChildByName("ll7_panel").setVisible(true);
				this.gameconfig_panel.getChildByName("slider_panel").setPositionY(this.gameconfig_panel.height*0.5);
			}else{
				this.gameconfig_panel.getChildByName("ll7_panel").setVisible(false);
	...
	init_ll7_panel:function(){
		var ll7_panel = this.gameconfig_panel.getChildByName("ll7_panel");
		var add_func = function(i){
			cc.log("选择ll7背景图",i);
			cc.sys.localStorage.setItem("LL7_BG_JSON", '{"now_bg":'+ i +'}');
			if(h1global && h1global.curUIMgr && h1global.curUIMgr.roomLayoutMgr && h1global.curUIMgr.roomLayoutMgr.backgroundStrategy){
				h1global.curUIMgr.roomLayoutMgr.backgroundStrategy.imgPath = h1global.curUIMgr.roomLayoutMgr.backgroundStrategy.imgPathList[i];
				h1global.curUIMgr.roomLayoutMgr.backgroundStrategy.updateBackground();
			}
		};
		...
		var info_dict = eval("(" + info_json + ")");
		cc.log(info_dict["now_bg"]);
		var select_id = info_dict["now_bg"];

		UICommonWidget.create_check_box_group(ll7_panel,"bg_chx",2, add_func, select_id, "img");
	},
});