...
var PlayBackUI = MultipleLayoutUI.extend({
    ctor: function () {
        this._super();
	    this.resourceFilename = PlayBackUI.getResourceFile(cc.sys.localStorage.getItem(cutil.keyConvert("GAME_ROOM_UI", this.gameType)));
        this.op_chk_list = {
    ...
    _copyRoomInfoToBackground: function () {
	    ...
	    copy_info_panel.setPositionY(info_panel.getPositionY() + 40);
    ...
    showOperationPanel: function (serverSitNum, ops, doOp) {
	    if (doOp === const_tylsmj.DISCARD_FORCE) {return;}
	    let player = h1global.player();
        let index = player.server2CurSitNum(serverSitNum);
        var cur_player_operation_panel = null;
	    if (player.curGameRoom.player_num === 3) {
		    for (var i = 0; i < 3; i++) {
			    var idx = i === 2 ? 3 : i;
			    cur_player_operation_panel = this.rootUINode.getChildByName('player_operation_panel' + idx);
			    cur_player_operation_panel.stopAllActions();
			    cur_player_operation_panel.setVisible(idx === index);
		    }
		    this.rootUINode.getChildByName('player_operation_panel2').setVisible(false);
	    } else {
