var GPSUI = UIBase.extend({
    ctor:function() {
	    ...
	    this.setLocalZOrder(const_val.GPSUIZOrder);
    },
    ...
    initUI:function(){
        ...
        for(var i = 0 ; i < 4 ; i++){
			...
			flash_point_img && flash_point_img.runAction(cc.RepeatForever.create(cc.Sequence.create(
                    cc.FadeIn.create(0.5 + i * 0.1),
                    cc.FadeOut.create(0.5)
                ))
            );
        }
        this.check_list = ["系统防作弊检测开始\n\n系统防外挂检测开始","玩家IP地址检测中...\n\n系统外挂检测中...","玩家定位距离检测中...\n\n系统外挂检测中..."];
    ...
    check_ip_same:function () {
        ...
        for(let i = 0 ; i < idx_ip_list.length ; i++){
            cutil.loadPortraitTexture(playerInfoList[idx_ip_list[i]]["head_icon"], playerInfoList[idx_ip_list[i]]["sex"], function(img){
                if(h1global.curUIMgr.gps_ui && h1global.curUIMgr.gps_ui.is_show){
                    ...
                    var size = 50;
                    if (idx_ip_list.length === 4) {
	                    size = 43;
	                    portrait_sprite.x = self.gps_panel.getContentSize().width * (0.12 + 0.11 * i + idx_ip_list.length % 3 * 0.05);
                    } else {
	                    size = 50;
                    ...
                    portrait_sprite.runAction(cc.Sequence.create(
                        ...
                        cc.EaseIn.create(cc.scaleTo(0.5, size/portrait_sprite.getContentSize().width), 0.3),
                        cc.CallFunc.create(function () {
        ...
        self.gps_panel.getChildByName("check_panel").setVisible(true);
        for(var i = 0 ; i < idx_distance_list.length ; i++){
