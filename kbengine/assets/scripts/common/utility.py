...
def Request(url, func):
	async = KBEngine.globalData["AsyncRequest"]
...
def get_user_info(accountName, callback):
	...
	Request(url + suffix, lambda x: callback(x))
...
def update_card_diamond(accountName, deltaCard, deltaDiamond, callback, reason=""):
	ts = get_cur_timestamp()
	to_sign = accountName + "_" + str(ts) + "_" + str(deltaCard) + "_" + str(
	...
	Post(url, data, lambda x: callback(x))

def post_award_msg(accountName, callback, reason=""):
	ts = get_cur_timestamp()
	to_sign = accountName + "_" + reason + "_" + str(ts) + "_" + switch.PHP_SERVER_SECRET
	# DEBUG_MSG("to sign::" + to_sign)
	sign = get_md5(to_sign)
	# DEBUG_MSG("MD5::" + sign)
	url = switch.PHP_SERVER_URL + 'record_award'
	data = {
		"timestamp": ts,
		"unionid": accountName,
		"sign": sign,
...
def update_data_statistics(ts, avatar_num, online_num, room_num, callback):
	to_sign = const.GAME_NAME + "_" + str(ts) + "_" + str(avatar_num) + "_" + str(online_num) + "_" + str(
		room_num) + "_" + switch.PHP_SERVER_SECRET
...
def update_dau(dau, game_name, callback):
	ts = get_cur_timestamp()
	to_sign = game_name + "_" + str(ts) + "_" + str(dau) + "_" + switch.PHP_SERVER_SECRET
...
# 获取测试模式 初始信息
def getDebugPrefab(owner, callback, debug_game_name):
	ts = int(time.mktime(datetime.now().timetuple()))
	url = '{}?timestamp={}&from=py&game={}&owner={}'.format(switch.PHP_DEBUG_URL, ts, debug_game_name, owner)
	import urllib.request
	try:
		resp = urllib.request.urlopen(url, timeout=2)
		data = resp.read().decode()
		DEBUG_MSG("{} {} debugmode,content:{}".format(owner, debug_game_name, data))
		if data is None or data == "10000" or data[0:2] != "ok":  # 10000代表找不到该文件
			callable(callback) and callback(None)
		else:
			callable(callback) and callback(json.loads(data[2:]))
	except:
		# import traceback
		# err, msg, stack = sys.exc_info()
		# DEBUG_MSG("exc_info: {}, {}, {}".format(err, msg, traceback.extract_tb(stack)))
		callable(callback) and callback(None)
...
# 计算房卡消耗
def calc_cost(game_type, params):
	if game_type in const.GameTypeTotal:
		pay_mode = params['pay_mode']
		game_round = params['game_round']
		room_type = params['room_type']

		if game_type == const.DouDiZhu:
			base = int(game_round / 4) + 1
			AAbase = int(game_round / 8)
		elif game_type == const.LvLiangKDDMJ:
			if pay_mode == const.AA_PAY_MODE:
				if int(game_round) == 8:
					AAbase = 3
				elif int(game_round) == 16:
					AAbase = 6
			else:
				if int(game_round) == 4:
					base = 4
				elif int(game_round) == 8:
					base = 8
				elif int(game_round) == 12:
					base = 11
		elif game_type == const.LvLiang7:
			if game_round == 6:
				base = 5
			elif game_round == 8:
				base = 7
				...
				base = 10
			AAbase = 1 if game_round == 6 else 2
		else:
			base = int(game_round / 4) + 1
			AAbase = int(game_round / 8)

		if room_type == const.CLUB_ROOM:
			if pay_mode == const.AA_PAY_MODE:
				return AAbase, 9999
			else:
				return base, 9999
		elif room_type == const.NORMAL_ROOM:
			if pay_mode == const.AA_PAY_MODE:
				return AAbase, 9999
			else:
				return base, 9999
