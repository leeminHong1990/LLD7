...
class iDAU:
	...
	def addDailyRound(self, round = 1):
		if utility.is_same_day(self.dailyRoundTime, time.time()):
			self.dailyRound += round
		else:
			self.dailyRound = round
			self.dailyRoundTime = int(time.time())

		self.event_mgr.push_event(Events.EVENT_CLUB_DAILY_ROUND_CHANGE, self.dailyRound)