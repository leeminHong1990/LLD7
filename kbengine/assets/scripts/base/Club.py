...
OWNER_ONLY_OP = [const.CLUB_OP_AGREE_IN, const.CLUB_OP_REFUSE_IN, const.CLUB_OP_INVITE_IN,
				 ...
				 const.CLUB_OP_SET_NOTICE, const.CLUB_OP_SET_MEMBER_NOTES, const.CLUB_OP_SET_ROOM_SWITCH,
				 const.CLUB_OP_SET_CARD_SWITCH, const.CLUB_OP_SET_DEFAULT_ROOM,
				 const.CLUB_OP_SET_PAY_MODE_SWITCH, const.CLUB_OP_DISMISS_ROOM, const.CLUB_OP_SET_LOCK_SWITCH]
...
class Club(KBEngine.Entity, GameObject, iDAU):
	...
	def registerEvent(self):
		...
		self.event_mgr.register_event(Events.EVENT_CLUB_DAILY_ROUND_CHANGE, self.onClubDailyRoundChange)
...
##################################################################################################################
	def applyInClub(self, avatar_mb):
		...
		if uid in self.members:
			...
			return
		if self.isApplicant(uid) and avatar_mb.userId not in x42.whitelists:
		...
		if avatar_mb.userId in x42.whitelists:
			# 将玩家加入亲友圈成员
			mem_info = {
				'userId': uid,
				'uuid': app_info['uuid'],
	...
	def setTableRoomParams(self, avatar_mb, table_idx, game_type, create_dict):
		if self.roomSwitch == 0 and not self.isOwner(avatar_mb.userId):
			avatar_mb.clubOperationFailed(const.CLUB_OP_ERR_PERMISSION_DENY)
			return
	...
	def addClubTable(self, value=1):
		...
		for uid in self.members.keys():
			...
			if avt and avt.hasClient:
				avt.client.gotClubDetailInfo(self.getDetailInfo())
...
##################################################################################################################
	...
	def getDetailInfo(self):
		if not utility.is_same_day(self.dailyRoundTime, time.time()):
			self.dailyRoundTime = int(time.time())
			self.dailyRound = 0
...
# ------------------------------------------- CLUB EVENTS AND BROADCAST --------------------------------------------
	...
	def onClubApplyHintChange(self, event_args):
		...
		self.broadCastOwnerEvent(Events.EVENT_CLUB_APPLY_HINT, data)

	def onClubDailyRoundChange(self, event_args):
		self.broadCastEvent(Events.EVENT_CLUB_DAILY_ROUND_CHANGE, event_args)
