	...
	def charge(self, accountList):
		...
		if self.pay_mode == const.NORMAL_PAY_MODE:
			...
			def pay_callback(content):
				...
				DEBUG_MSG("player {} charge NORMAL_PAY_MODE pay_callback content:{}".format(pay_account, content))
				# 不论是否扣卡成功, 都让玩家继续游戏
				self.chargeCallback(True)
			...
			def pay_callback(content):
				# ret = self._check_pay_callback(content)
				# self.chargeCallback(ret)
				DEBUG_MSG("player {} charge CLUB_PAY_MODE pay_callback  content:{}".format(pay_account, content))
				# 不论是否扣卡成功, 都让玩家继续游戏
				self.chargeCallback(True)
			...
			def pay_callback(content):
				...
				# self.chargeCallback(ret)
				DEBUG_MSG("player {} charge AA_PAY_MODE pay_callback  content:{}".format(str(accountList), content))
				# 不论是否扣卡成功, 都让玩家继续游戏
				self.chargeCallback(True)
